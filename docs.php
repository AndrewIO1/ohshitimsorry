    ...
    <body>
        ...
        <button class="btn btn-default" type="button">Поиск</button>
				...
				if (isset($_SESSION['id']) and $_SESSION['role'] == 99){
					echo '<p><a href="docload_form.php">Добавить материал</a></p>';
				}
				?> 
      ...
      -->
                ...
                <div class="col-sm-8 text-left col-lg-10">
                   <table width="800" class="center-block">
                   <style>
					   th {
						   padding: 4px;
					   }
                   ...
                   <p><h1 class="text-center">Учебники</h1></p>
                   <hr>
		...
		$dir = "http://ayylmaokek.azurewebsites.net/uploads/files/";
					
					$q="SELECT * FROM docs ORDER BY id DESC ";
...
while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
	...
	echo "<tr>";
    echo "<th class='angel'>
	<p><h4>".$name. ", " .$author.". " .$year. " г.</h4></p> </th>";
	if (isset($row['link'])){
		echo "<th><a href='".$link."' role='button' class='btn btn-primary '><span class='glyphicon glyphicon-eye-open' aria-hidden='true'></span>Просмотр</a></th>";
		
		
	}
	else {
		echo "<th><a href='".$dir.$filedir."' role='button' class='btn btn-primary '><span class='glyphicon glyphicon-eye-open' aria-hidden='true'></span>Просмотр</a></th>";
	echo "<th><a download href='".$dir.$filedir."' role='button' class='btn btn-default '><span class='glyphicon glyphicon-download-alt' aria-hidden='true'></span>Скачать</a></th>";
	//echo "<th>
	//<form action='docs_view.php' method='post'>
	//<input type='hidden' name='id' value='".$id."'>
	//<input type='hidden' name='filedir' value='".$filedir."'>
	//<input type='submit' class='btn btn-primary' role=button value='Просмотр'>
	//</form>
	//</th>";
	
	}
	if (isset($_SESSION['role']) and $_SESSION['role'] == 99){
	echo "<th>
	<form action='docs_edit.php' method='post'>
	<input type='hidden' name='id' value='".$id."'>
	<input type='hidden' name='filedir' value='".$filedir."'>
	<input type='submit' class='btn btn-info' role=button value='Изменить'>
	</form>
	</th>";
	echo "<th>
